...
def read():
    global rain
    df = pd.read_excel(r'D:\Desktop\three.xls')
...
if __name__ == "__main__":
    ...
    with open("D:\Desktop\\rain.csv", "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerows(haha)
    # # 保存图为svg格式，即矢量图格式
    # for i in range(len(rain)):
    #     rain[i] = rain[i] / 10
    #     if rain[i] > 2.5 and rain[i] < 5:
    #         rain[i] = 0
    # rain = rain[:end]
    # list = [i for i in range(len(rain))]
    #
    #
    # plt.plot(list, rain, color="#5095B0" ,linewidth=1.0, linestyle="-", label = '阅读人数')
    # plt.xlabel('time(y)')
